...
# Load raster img
img = raster.read([i+1 for i, band in enumerate(raster.descriptions) if (band[:4] == "2020") and (int(band[4:6]) >= df_vew["final_plant_date"].min().month)])
list_band = [band for i, band in enumerate(raster.descriptions) if (band[:4] == "2020") and (int(band[4:6]) >= df_vew["final_plant_date"].min().month)]
#%%
