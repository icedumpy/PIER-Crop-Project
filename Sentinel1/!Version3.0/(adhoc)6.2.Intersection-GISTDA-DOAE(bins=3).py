...
for p in pbar:
    ...
    img = np.where((img_DOAE == 0) & (img_GISTDA == 1), 2, img)
    img = np.where((img_DOAE == 1) & (img_GISTDA == 1), 3, img)
    
    # Some process 2
    TPR = (img == 3).sum()/((img == 1) | (img == 3)).sum()
    dict_tpr[p] = TPR
    #%%
    ...
    show(img, transform=transform, cmap=cmap, ax=ax, vmin=0, vmax=3)
    ax.set_title(f'{gdf_thailand.loc[f"TH{p}", "ADM1_EN"]}\nTPR: {TPR:.4f}')
    ax.legend(custom_lines, ['(DOAE == 1) & (GISTDA == 0)', 
                             '(DOAE == 0) & (GISTDA == 1)', 
                             '(DOAE == 1) & (GISTDA == 1)'], loc=4)

    # Save plot
    pylab.get_current_fig_manager().window.showMaximized()
    fig.savefig(os.path.join(r"F:\CROP-PIER\CROP-WORK\Presentation\20201118\DOAExGISTDA\DOAE_GISTDA", f'{p}_{gdf_thailand.loc[f"TH{p}", "ADM1_EN"]}.png'), 
                dpi=1000)
#%%
import pandas as pd
df = pd.DataFrame(pd.Series(dict_tpr))
df.to_csv(r"F:\CROP-PIER\CROP-WORK\Presentation\20201123\Slide6\tpr.csv")