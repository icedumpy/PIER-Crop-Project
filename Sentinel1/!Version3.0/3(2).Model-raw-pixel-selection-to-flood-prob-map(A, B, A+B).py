...
def get_df_train_test(root_df_flood, root_df_nonflood, root_df_mapping, p, strip_id):
    ...
    columns_pixel_values = columns_pixel_values_new
...
if sat_type == "S1AB":
    ...
    repeat_cycle = 12
#%% Define parameters
for strip_id in tqdm(["302", "303", "304", "305", "401", "402", "403"]):
# for strip_id in ["304", "402", "403"]:
    ...
    if model_strip_id == "all":
         root_raster_flood = os.path.join(r"G:\!PIER\!Flood-map\Sentinel-1", sat_type, "Prob", f"{strip_id}-all-modified")
         path_model = os.path.join(root_model, sat_type, "all-modified_RF_raw_pixel_values.joblib")
    else:
        root_raster_flood = os.path.join(r"G:\!PIER\!Flood-map\Sentinel-1", sat_type, "Prob", f"{strip_id}-modified")
        path_model = os.path.join(root_model, sat_type, f"{strip_id}-modified", f"{strip_id}_RF_raw_pixel_values.joblib")    
    os.makedirs(root_raster_flood, exist_ok=True)
    #%% Load model
    if os.path.exists(path_model):
        if model_strip_id == "all":
            path_roc_params = os.path.join(os.path.dirname(path_model), "all-modified_RF_raw_pixel_values_roc_params.h5")
        else:
    ...
    for i in range(0, len(df_raster_info)+1-length_vector):
        ...
        list_band = [int(df_raster_info.at[j, "band"]) 
                     if not np.isnan(df_raster_info.at[j, "band"]) 
                     else df_raster_info.at[j, "band"]
                     for j in range(i, i+length_vector)]
        list_date = [df_raster_info.at[j, "ideal_date"] for j in range(i, i+length_vector)]
        
        # Skip if already exist
        path_raster_flood = os.path.join(root_raster_flood, str(list_date[-1].date())+".tif")
        if os.path.exists(path_raster_flood):
            continue
    
        # Show details
        print(f"{strip_id}: {str(list_date[-1].date())}")
    ...
    create_vrt(path_save=os.path.join(root_raster_flood, "flood_map-modified.vrt"),
