...
path_row = lambda file, root_raster_rowcol: os.path.join(root_raster_rowcol, f"s1_pixel_row_map_{os.path.basename(os.path.dirname(file))}.tiff")
path_col = lambda file, root_raster_rowcol: os.path.join(root_raster_rowcol, f"s1_pixel_col_map_{os.path.basename(os.path.dirname(file))}.tiff")
...
def create_vrt(path_vrt, list_path_raster, list_name=None):
    list_path_raster = [path_row(list_path_raster[0], root_raster_rowcol), 
                        path_col(list_path_raster[0], root_raster_rowcol)]  + list_path_raster
...
if __name__ == "__main__":
    ...
    # เอาไว้เซฟไฟล์ใน drive c เพื่อจะได้เอาไปใช้ต่อง่ายๆ เพราะมันไว (ssd)
    ...
    # Run เสร็จก็จะได้ไฟล์ .vrt ที่สแตคกันละ
    # S1A
    #     - S1A_101.vrt
    #     - S1A_102.vrt
    #     - ...
