...
for include_age1 in [False, True]:
    ...
    df = assign_sharp_drop(df, include_age1=include_age1)
    print("Assign sharpdrop 2020")
    df_2020 = assign_sharp_drop(df_2020, include_age1=include_age1)
    ...
    df[['drop_t0', 'drop_t1', 'drop_t2', 'drop_t3', 'drop_t4', 'drop_t5']] = df[['drop_t0', 'drop_t1', 'drop_t2', 'drop_t3', 'drop_t4', 'drop_t5']].subtract(df['drop_t-1'], axis=0)
    #%%
    ...
    #%%
    x_train = df.loc[(df["year"].isin([2018, 2019])), model_parameters].values
    x_test = df.loc[(df["year"].isin([2020])), model_parameters].values
    
    y_train = df.loc[(df["year"].isin([2018, 2019])), "label"].values
    y_test = df.loc[(df["year"].isin([2020])), "label"].values
    #%%
    # Retrain model
