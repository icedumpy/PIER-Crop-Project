...
for strip_id in ["302", "303", "304", "305", "401", "402", "403"]:
    ...
    threshold = get_threshold_of_selected_fpr(dict_roc_params["fpr"], dict_roc_params["threshold_roc"], selected_fpr=0.3)
    ...
    df = df.assign(threshold = threshold)
    df = df.assign(strip_id = strip_id)
    df = df[["PAT", "strip_id", "ext_act_id", "loss_ratio", "predict_proba", "threshold", "predict"]]
    df.to_parquet(os.path.join(r"F:\CROP-PIER\CROP-WORK\Sentinel1_dataframe_updated\s1ab_for_report_20210625", f"{strip_id}.parquet"))
    #%%
