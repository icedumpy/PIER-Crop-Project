...
path_rct = r"F:\CROP-PIER\CROP-WORK\RCT_tambonDisaster.csv"
...
df_rct = pd.read_csv(path_rct)
df_province_code = pd.read_csv(path_province_code)

df_rct["full_tambon_name"] = df_rct["province"] + "_" + df_rct["amphur"] + "_" + df_rct["tambon"]
df_province_code = df_province_code[df_province_code["full_tambon_name"].isin(df_rct["full_tambon_name"])]
df_province_code["PAT"] = df_province_code['PROVINCE_CODE'].astype(str).str.zfill(2) + df_province_code['AMPHUR_CODE'].astype(str).str.zfill(2) + df_province_code['TAMBON_CODE'].astype(str).str.zfill(2)
# %%
...
# strip_id = "304"
    ...
    #%%
    df = df[["PAT", "ext_act_id", "loss_ratio", "predict_proba", "predict"]]
    df["threshold"] = threshold
    df["strip_id"] = strip_id
    print(df)
    #%%
    df.to_parquet(rf"C:\Users\PongporC\Desktop\RCT\{strip_id}.parquet")
