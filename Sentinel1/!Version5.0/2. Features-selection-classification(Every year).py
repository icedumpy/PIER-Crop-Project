...
def assign_drop_rise(df):
    ...
    df = df.assign(**{
        ...
        "sharp_drop_lag3" : df[columns_age1[-3:]+columns_age2+columns_age3+columns_age4].diff(periods=2, axis=1).min(axis=1),
        "sharp_drop_column_lag1" : df[columns_age1[-1:]+columns_age2+columns_age3+columns_age4].diff(periods=1, axis=1).idxmin(axis=1),
        "sharp_drop_column_lag2" : df[columns_age1[-2:]+columns_age2+columns_age3+columns_age4].diff(periods=2, axis=1).idxmin(axis=1),
        "sharp_drop_column_lag3" : df[columns_age1[-3:]+columns_age2+columns_age3+columns_age4].diff(periods=2, axis=1).idxmin(axis=1)
    })
    ...
    df = df.assign(**{
        ...
        "sharp_rise_lag3" : df[columns_age1[-3:]+columns_age2+columns_age3+columns_age4].diff(periods=2, axis=1).max(axis=1),
        "sharp_rise_column_lag1" : df[columns_age1[-1:]+columns_age2+columns_age3+columns_age4].diff(periods=1, axis=1).idxmax(axis=1),
        "sharp_rise_column_lag2" : df[columns_age1[-2:]+columns_age2+columns_age3+columns_age4].diff(periods=2, axis=1).idxmax(axis=1),
        "sharp_rise_column_lag3" : df[columns_age1[-3:]+columns_age2+columns_age3+columns_age4].diff(periods=2, axis=1).idxmax(axis=1)
    })
    return df
# %%
...
for strip_id in ["402", "403"]:
    ...
    #%%
    model_parameters_1 = [
        "sharpest_drop_period", 
        'drop_t-2', 'drop_t-1', 'drop_t0', 'drop_t1', 'drop_t2', 
        'drop_t3', 'drop_t4', 'drop_t5',
        'sharp_drop_lag1', 'sharp_drop_lag2', 'sharp_drop_lag3',
        'sharp_rise_lag1', 'sharp_rise_lag2', 'sharp_rise_lag3'
    ]
    model_parameters_2 = [
        "sharpest_drop_period", 'sharp_drop_lag1',
        'drop_t-2', 'drop_t-1', 'drop_t0', 'drop_t1', 'drop_t2', 
        'drop_t3', 'drop_t4', 'drop_t5',
    ]
    #%%
    ...
    for year, color in zip([2018, 2019, 2020], ["r-", "g-", "b-"]):
        try:
            x = df.loc[(~(df["ext_act_id"]%10).isin([8, 9])) & (df["final_crop_year"] == year), model_parameters_1].values
    ...
    fig.savefig(os.path.join(root_model, f"{strip_id}_ROC_separate_1.png"), bbox_inches='tight')
    ...
    ax = set_roc_plot_template(ax)
    ax.set_title(f'ROC Curve: {strip_id}\nAll_touched(False), Tier(1,)\nTrain samples: Flood:{(y_train == 1).sum():,}, Non-Flood:{(y_train == 0).sum():,}\nTest samples: Flood:{(y_test == 1).sum():,}, Non-Flood:{(y_test == 0).sum():,}')
    fig.savefig(os.path.join(root_model, f"{strip_id}_ROC_1.png"), bbox_inches='tight')
    #%%
    path_model = os.path.join(root_model, f"{strip_id}_1.joblib")
    ...
    dict_roc_params = {"fpr":fpr,
                       ...
                       "y_train":y_train}
    save_h5(os.path.join(os.path.dirname(path_model), f"{strip_id}_metrics_params_1.h5"), dict_roc_params)
    #%%
    # =============================================================================
    # Parameters_2    
    # =============================================================================
    ...
    for year, color in zip([2018, 2019, 2020], ["r-", "g-", "b-"]):
        try:
            x = df.loc[(~(df["ext_act_id"]%10).isin([8, 9])) & (df["final_crop_year"] == year), model_parameters_2].values
            y = df.loc[(~(df["ext_act_id"]%10).isin([8, 9])) & (df["final_crop_year"] == year), 'label'].values
            ax, _, _, _, _, _ = plot_roc_curve(model, x, y, color=color, label=f"Train{year}({len(df.loc[(~(df['ext_act_id']%10).isin([8, 9])) & (df['final_crop_year'] == year)]):,})", ax=ax)
        except:
            pass
    ...
    ax = set_roc_plot_template(ax)
    ax.set_title(f'ROC Curve: {strip_id}\nAll_touched(False), Tier(1,)\nTrain samples: Flood:{(y_train == 1).sum():,}, Non-Flood:{(y_train == 0).sum():,}\nTest samples: Flood:{(y_test == 1).sum():,}, Non-Flood:{(y_test == 0).sum():,}')
    fig.savefig(os.path.join(root_model, f"{strip_id}_ROC_separate_2.png"), bbox_inches='tight')
    ...
    ax = set_roc_plot_template(ax)
    ax.set_title(f'ROC Curve: {strip_id}\nAll_touched(False), Tier(1,)\nTrain samples: Flood:{(y_train == 1).sum():,}, Non-Flood:{(y_train == 0).sum():,}\nTest samples: Flood:{(y_test == 1).sum():,}, Non-Flood:{(y_test == 0).sum():,}')
    fig.savefig(os.path.join(root_model, f"{strip_id}_ROC_2.png"), bbox_inches='tight')
    #%%
    path_model = os.path.join(root_model, f"{strip_id}_2.joblib")
    save_model(path_model, model)
    ...
    dict_roc_params = {"fpr":fpr,
                       ...
                       "y_predict_prob_roc":y_predict_prob_roc[:, 1],
                       "y_train":y_train}
    save_h5(os.path.join(os.path.dirname(path_model), f"{strip_id}_metrics_params_2.h5"), dict_roc_params)
#%%
# Evaluate out-sample
