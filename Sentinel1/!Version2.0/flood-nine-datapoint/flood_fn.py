...
def ls8_rgb(root):
    for file in os.listdir(root):
        ...
        if band in ['B2', 'B3', 'B4']:
            ...
            if band == 'B2':
                blue = rasterio.open(path_file).read()
            if band == 'B3':
                green = rasterio.open(path_file).read()
            if band == 'B4':
                red = rasterio.open(path_file).read()
                
    blue = exposure.equalize_hist(blue)
    green = exposure.equalize_hist(green)
    red = exposure.equalize_hist(red)
    
    rgb = np.concatenate((red, green, blue), axis=0)
...
def sampling_dataset(root, flood = True, sampling_size = 0.1):
    ...
    for filename in os.listdir(root):
        ...
        # Clean: Drop nan
        df = df[~np.any(df.iloc[:, 8:].isna(), axis=1)]
...
def load_dataset_df(root_df_dry, root_df_flood, index_name, sampling_size = 1):
    ...
    if index_name == 'all':
        ...
        # Clean: Drop nan
        df_flood = df_flood[~np.any(df_flood.iloc[:, 8:].isna(), axis=1)]
        df_dry = df_dry[~np.any(df_dry.iloc[:, 8:].isna(), axis=1)]
        
        # Clean: Drop low loss raio (only [0.8, 1] remain)
        df_flood = df_flood[(df_flood['loss_ratio']>=0.8) & (df_flood['loss_ratio']<=1)]
        
        # Clean 3: Drop duplicate
        df_flood = df_flood.drop_duplicates(subset=df_flood.columns[8:])
        df_dry = df_dry.drop_duplicates(subset=df_dry.columns[8:])
...
def load_vew_mapping_df(root_mapping, root_vew, index_name, date_range):
    ...
    # Replace no START_DATE data with NaT
...
def get_fuzzy_confusion_matrix(predicted, ref):
    ...
    where
        "predicted" is a list of predicted loss ratio, ex. [0.5, 0.0, 1.0, 0.8, 0.6]
        "ref" is a list of groundtruth loss ratio,     ex. [0.7, 1.0, 0.5, 0.4, 0.6]
    '''
    '''
                          Ref
                   |Flooded|No Flood|
...
def train_model(df_dry, df_flood, balanced = True, sample_size = None, verbose=1):
    ...
    if balanced:
        ...
        samples_dry = df_dry.iloc[:, 8:].sample(n=sample_size)
        samples_flood = df_flood.iloc[:, 8:].sample(n=sample_size)
        
    else:
        if sample_size == None:
            sample_size = 1
        samples_dry = df_dry.iloc[:, 8:].sample(frac=sample_size)
        samples_flood = df_flood.iloc[:, 8:].sample(frac=sample_size)
...
def create_flood_map(path_save, model, path_raster, path_mask, root_mapping, root_vew, index_name, threshold):
    raster = gdal.Open(path_raster)
    raster_date = get_raster_date(raster, dtype='gdal')    
    
    tqdm.write(f"Loading rice mask")
    if (not os.path.exists(path_mask)) and (not os.path.exists(path_mask.replace('.shp', '.tif'))):
    ...
    # Delete row_rice and col_rice at the edge of image
    row_edge_pos = row_rice==(mask_rice.shape[0]-1)
    col_edge_pos = col_rice==(mask_rice.shape[1]-1)
    edge_pos = (row_edge_pos | col_edge_pos)
    row_rice = row_rice[~edge_pos]
    col_rice = col_rice[~edge_pos]
    
    raster_im = raster.ReadAsArray()
    flood_im = np.zeros_like(raster_im, dtype=np.uint8)
    ...
    for band in tqdm(range(2, raster.RasterCount-2)):
        ...
        if (raster_date[band+1]-raster_date[band-2]).days == 36:
            ...
            # Get details for each pixel (nine datapoints)
            error_idx = []
            flood_details = np.zeros((len(row_rice), 36))
            for idx, (row, col) in enumerate(zip(row_rice, col_rice)):
                flood_detail = raster_im[band-2:band+2, row-1:row+2, col-1:col+2]
                try:
            ...
            # If error (can't get all pixel details (af the very edge of image)), del row, col and flood_details
            if len(error_idx)!=0:
                flood_details = np.delete(flood_details, error_idx, axis=0)
                row_rice = np.delete(row_rice, error_idx, axis=0)
                col_rice = np.delete(col_rice, error_idx, axis=0)
        
            pred = model.predict_proba(flood_details)
            pred_thresh = (pred[:, 1]>=threshold).astype('uint8')
            flood_im[band, row_rice, col_rice] = pred_thresh # [0 non-flood, 1 flood, 2 no data]
            flood_im[flood_im==0] = 0
