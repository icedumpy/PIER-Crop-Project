...
def extract_features(df, columns_temporal):
    ...
    for ext_act_id, df_grp in tqdm(df.groupby("ext_act_id")):
        ...
        # Get data (flood_column-lag -> flood_column-lag+4)
        columns_for_feature = [f"t{i}" for i in range(max(0, flood_column-periods), min(15, flood_column-periods+4))]
        ...
        for threshold in list_thresholds:
            ...
            dict_extracted_features = {
                **dict_extracted_features,
                **{ f"pct_of_plot_with_backscatter_under({threshold})_1-12_days_strict": pct_plot_1_12_strict,
                    f"pct_of_plot_with_backscatter_under({threshold})_13-24_days_strict": pct_plot_13_24_strict,
                    f"pct_of_plot_with_backscatter_under({threshold})_25+_days_strict": pct_plot_25_strict,
                    f"pct_of_plot_with_backscatter_under({threshold})_1-12_days_relax": pct_plot_1_12_relax,
                    f"pct_of_plot_with_backscatter_under({threshold})_13-24_days_relax": pct_plot_13_24_relax,
                    f"pct_of_plot_with_backscatter_under({threshold})_25+_days_relax": pct_plot_25_relax,
                  }
            }
...
root_save = r"F:\CROP-PIER\CROP-WORK\Sentinel1_dataframe_updated\s1a_features(same-idea-as-GISTDA-Flood)"
os.makedirs(root_save, exist_ok=True)

df_info = get_temporal_file_info(root_temporal)
columns_temporal = [f"t{i}" for i in range(15)]
#%%
print(f"Total files: {len(df_info)}")
for idx, file_info in df_info.iterrows():
    ...
    # Extract Features
    df_plot = extract_features(df, columns_temporal)
