...
def extract_features(df, columns, label):
    ...
    for ext_act_id, df_grp in tqdm(df.groupby("ext_act_id")):
        ...
        if (df_grp.iloc[0]["final_plant_date"].month < 7) and not (columns == ['t0', 't1', 't2', 't3', 't4', 't5', 't6']):
        ...
        columns_for_feature = [f"t{i}" for i in range(max(0, flood_column-periods), min(int(columns[-1][1:])+1, flood_column-periods+8))]
        ...
        for threshold in list_thresholds:
            ...
            dict_extracted_features = {
                **dict_extracted_features,
                **{ f"pct_of_plot_with_backscatter_under({threshold})_1-6_days_strict_{label}": pct_plot_1_6_strict,
                    f"pct_of_plot_with_backscatter_under({threshold})_7-12_days_strict_{label}": pct_plot_7_12_strict,
                    f"pct_of_plot_with_backscatter_under({threshold})_13-18_days_strict_{label}": pct_plot_13_18_strict,
                    f"pct_of_plot_with_backscatter_under({threshold})_19+_days_strict_{label}": pct_plot_19_strict,
                    f"pct_of_plot_with_backscatter_under({threshold})_1-6_days_relax_{label}": pct_plot_1_6_relax,
                    f"pct_of_plot_with_backscatter_under({threshold})_7-12_days_relax_{label}": pct_plot_7_12_relax,
                    f"pct_of_plot_with_backscatter_under({threshold})_13-18_days_relax_{label}": pct_plot_13_18_relax,
                    f"pct_of_plot_with_backscatter_under({threshold})_19+_days_relax_{label}": pct_plot_19_relax,
                  }
            }
